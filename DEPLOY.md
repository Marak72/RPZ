...
- **Поиск в SIEM идёт в фоне** (миграция `a3d92f1b8c57`, таблица
  `background_jobs`). Кнопка возвращает страницу сразу, ход работы показывает
  плашка в правом нижнем углу — она опрашивает `/soc/jobs/status.json`.
  Поднимать таймауты gunicorn и веб-сервера под поиск больше не нужно:
  запрос к порталу быстрый, долго работает фоновый поток внутри процесса.
  Отсюда следствие — при `systemctl restart soc-portal` незавершённые
  задания обрываются. Портал сам пометит их ошибкой, когда увидит, что
  исполнитель перестал отвечать (10 минут), и поиск можно будет запустить
  заново.
- **Таймаут самого запроса в SIEM** задаётся в разделе «Настройки» сервиса
  (по умолчанию 120 с) — это ожидание ответа SIEM, а не веб-сервера.
- **Настройка «максимум событий на домен»** (миграция `b4e07c2a91d3` её не
  трогает — это новый ключ `siem_max_events`, по умолчанию 20000). Прежний
  `siem_limit` ограничивал строки одного ответа; теперь события дочитываются
  страницами, и старое значение под новым смыслом означало бы тихое урезание
  выборки, поэтому ключ заведён отдельный. Прежнее значение можно не переносить.
- **Полная выгрузка SkyDNS** («Собрать всё») делает по отчёту на каждую
  отслеживаемую категорию и каждые сутки периода. Девять категорий за неделю —
  63 отчёта, каждый строится несколько секунд, так что прогон занимает минуты
  и идёт фоновым заданием. Обычная кнопка «Забрать из SkyDNS» осталась: один
  отчёт, быстро, но только вершина списка.
- **Правила исключений** (таблица `domain_exclusions`, миграция
  `b4e07c2a91d3`) хранятся в базе и настраиваются в разделе «Исключения»
  сервиса. На этапе развёртывания ничего не требуется.
- **Задание «зависло» после перезапуска службы.** Поток-исполнитель
  перезапуск не переживает, а запись о задании остаётся «выполняется» и
  блокирует запуск следующего. Снять её можно тремя способами:
  кнопкой «Отменить» на плашке задания; командой
  `flask jobs-reset` (снимает все незавершённые); либо ничего не делать —
  через 10 минут молчания портал закроет задание сам. Останавливать
  сервисы или чистить базу руками не нужно.
- **`database is locked` при сохранении во время фоновой выгрузки.** SQLite
  пускает только одного писателя. Портал держит свои транзакции короткими
  (запись частями по 200 записей) и работает в режиме журнала WAL с ожиданием
  30 секунд — при этом чтение страниц не блокируется вовсе, а запись ждёт
  своей очереди. Если ошибка всё же появилась, посмотрите в журнале службы
  предупреждение «SQLite работает в режиме журнала … вместо WAL»: WAL не
  включается, когда файл базы лежит на сетевой файловой системе (NFS, CIFS).
  В этом случае перенесите `instance/rpz.db` на локальный диск.
- **Домены уходят в SIEM пачками** (по умолчанию 20 в одном запросе):
  условия объединяются через `or`, события раскладываются обратно порталом.
  Предела на число доменов за прогон больше нет. Если инсталляция плохо
  переносит длинный фильтр, поставьте в настройках «доменов в одном
  запросе» = 1 — это прежнее поведение, отдельный запрос на домен.
- **«Найдено хостов: 0»:** откройте «Настройки → Диагностика запроса в SIEM»
  и выполните запрос по одному домену. Страница покажет, сколько событий
  нашлось и как SIEM назвал колонки ответа: если строки есть, а хостов нет —
  поле группировки надо поправить (их можно перечислить через запятую).
