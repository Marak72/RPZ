...
class UploadForm(FlaskForm):
    """Загрузка писем ФСТЭК — как одного, так и сразу пачки.

    Реквизиты можно не заполнять: разбор сам достаёт номер и дату из текста
    письма и склеивает файлы одного письма в группу. Заполненный номер —
    указание «вся эта пачка относится к одному письму», оно сильнее
    автоматики и снимает всякое гадание.
    """

    documents = MultipleFileField(
        "Файлы писем (.docx / .odt / .pdf) — можно выбрать сразу много",
        validators=[
            FileAllowed(["docx", "odt", "pdf"], "Только файлы .docx, .odt и .pdf"),
        ],
    )
    letter_number = StringField(
        "Номер письма (необязательно — обычно определяется сам)",
        validators=[Optional()],
    )
    letter_date = DateField("Дата письма (необязательно)",
                            validators=[Optional()])
...
class AddFilesForm(FlaskForm):
    """Дослать файлы к уже заведённому письму (приложения приходят позже)."""

    documents = MultipleFileField(
        "Добавить файлы к письму",
        validators=[
            FileAllowed(["docx", "odt", "pdf"], "Только файлы .docx, .odt и .pdf"),
        ],
    )
    submit_files = SubmitField("Добавить файлы")
